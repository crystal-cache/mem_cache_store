...
describe Cache do
  context Cache::MemCacheStore do
    ...
    it "initialize" do
      store = Cache::MemCacheStore(String).new(expires_in: 12.hours)
    ...
    it "initialize with memcached" do
      ...
      store = Cache::MemCacheStore(String).new(expires_in: 12.hours, cache: memcached)

      store.should be_a(Cache::Store(String))
    end

    it "write to cache first time" do
      store = Cache::MemCacheStore(String).new(12.hours)
    ...
    it "don't fetch from cache if expired" do
      store = Cache::MemCacheStore(String).new(1.seconds)
    ...
    it "#exists? expires" do
      store = Cache::MemCacheStore(String).new(1.second)
    ...
    it "#increment" do
      store = Cache::MemCacheStore(Int32).new(12.hours)
